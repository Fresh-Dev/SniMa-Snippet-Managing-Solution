...
class User
{
        ...
        if($UsrCntRes==0){return false;}
            ...
            if(count($Rows)>0)
            {
                ...
                if($Userrow['password'] == $DecryptedPassword)
                {
                    ...
                    $this->UpdateLastLogin($Userrow[0]);
                    return true;
                }
                else{return false;}
            }
    ...
    {   
        if($this->CheckAuthkey($Authkey)==false || $this->CheckEmail($Email)==false || $this->CheckPasswords($Password, $PasswordRepeat)==false || $this->CheckUsername($Username)==false){return false;}
        else
        {
     ...
     * @return boolean
     */
    public function UpdateUser($ID,$NewEmail,$NewPassword,$NewPasswordRepeat)
    {
        if($this->CheckPasswords($NewPassword, $NewPasswordRepeat)!=false)
        {
            $EnryptedPassword = sperren_sha256_rijndael($NewPassword, SALZ, SALZ2);
            $Query = "UPDATE `user` SET  `password` =  '$EnryptedPassword',`email` =  '$NewEmail' WHERE `id` =$ID;";
    ...
    {
        $DB = \db::inst();$DB->query("UPDATE `user` SET  `lastLogin` =  '".time()."' WHERE `id` =$UserID;");
    }

    private function InsertUserdata($Username,$EnryptedPassword,$Email="")
    {
        $Query = "INSERT INTO `user` (`username`, `password`, `regDate`) VALUES ('$Username', '$EnryptedPassword', '".time()."');";$DB = \db::inst();$DB->query($Query);
    }
